...
export default function Stocks() {
	...
	const [searchPalet, setSearchPalet] = useState('');
	const [currentPaletPage, setCurrentPaletPage] = useState(1);
	const [totalPaletPages, setTotalPaletPages] = useState(0);
	const [totalPaletItems, setTotalPaletItems] = useState(0);

	// Modal states
	const [showPaletModal, setShowPaletModal] = useState(false);
	const [showDeletePaletModal, setShowDeletePaletModal] = useState(false);
	const [showScannerModal, setShowScannerModal] = useState(false);
	const [showBarcodeModal, setShowBarcodeModal] = useState(false);
	const [editingPalet, setEditingPalet] = useState<Palet | null>(null);
	const [paletToDelete, setPaletToDelete] = useState<Palet | null>(null);
	const [paletForBarcode, setPaletForBarcode] = useState<Palet | null>(null);
	const [isSubmitting, setIsSubmitting] = useState(false);

	const [paletFormData, setPaletFormData] = useState({
		name: '',
		location: '',
	});

	const fetchPalets = useCallback(async () => {
		setLoading(true);
		try {
			const response = await getPalets(currentPaletPage, 10, searchPalet);
			if (response.statusCode === 200) {
				setPalets(response.data);
				setTotalPaletPages(response.pagination.totalPages);
				setTotalPaletItems(response.pagination.totalItems);
				setError('');
			} else {
	...
	const handleSearchPalet = (e: React.FormEvent) => {
		e.preventDefault();
		setCurrentPaletPage(1);
		fetchPalets();
	};

	const handlePaletInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		const { name, value } = e.target;
		setPaletFormData((prev) => ({ ...prev, [name]: value }));
	};

	const resetPaletForm = () => {
		setPaletFormData({ name: '', location: '' });
	};

	const handleAddPalet = async (e: React.FormEvent) => {
		e.preventDefault();
		setIsSubmitting(true);
		setError('');

		try {
			const response = await createPalet(paletFormData);
			if (response.statusCode === 201) {
				fetchPalets();
				setShowPaletModal(false);
				resetPaletForm();
			} else {
				setError(response.error || 'Gagal menambah palet');
			}
		} catch {
	...
	const handleUpdatePalet = async (e: React.FormEvent) => {
		e.preventDefault();
		...
		try {
			const response = await updatePalet(editingPalet.id, paletFormData);
			if (response.statusCode === 200) {
				fetchPalets();
				setShowPaletModal(false);
				...
				setError(response.error || 'Gagal mengubah palet');
			}
		} catch {
	...
	const handleDeletePalet = async () => {
		if (!paletToDelete?.id) return;

		try {
			const response = await deletePalet(paletToDelete.id);
			if (response.statusCode === 200) {
				fetchPalets();
				setShowDeletePaletModal(false);
				setPaletToDelete(null);
			} else {
				setError(response.error || 'Gagal menghapus palet');
			}
		} catch {
			setError('Gagal menghapus palet');
		}
	};
	...
	const handlePrintBarcode = () => {
		...
		if (!printContent || !paletForBarcode) return;
	...
	return (
		<MainLayout title="Manajemen Stok Palet">
			<div className="max-w-7xl mx-auto">
				<div className="mb-8 flex justify-between items-center">
					<div>
						<h2 className="text-2xl font-bold text-gray-900 mb-2">Manajemen Stok Palet</h2>
						<p className="text-gray-600">Kelola palet dan inventaris stok</p>
					</div>
					<button
				...
				<div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 overflow-hidden">
					<div className="p-6 border-b border-gray-200">
						<div className="flex justify-between items-center mb-4">
							<h3 className="text-lg font-semibold text-gray-900">Daftar Palet</h3>
							<button
								onClick={() => {
									setEditingPalet(null);
									resetPaletForm();
						...
						<form onSubmit={handleSearchPalet} className="flex gap-2">
							<input
								type="text"
								value={searchPalet}
								onChange={(e) => setSearchPalet(e.target.value)}
					...
					<div className="overflow-x-auto">
						{loading ? (
							...
							<div className="p-8 text-center text-gray-500">Tidak ada palet</div>
						) : (
							<table className="min-w-full divide-y divide-gray-200">
								...
								<tbody className="bg-white divide-y divide-gray-200">
									{palets.map((palet) => (
										<tr key={palet.id} className="hover:bg-gray-50">
											<td className="px-6 py-4 whitespace-nowrap">
												<div className="text-sm font-medium text-gray-900">{palet.name}</div>
											</td>
											<td className="px-6 py-4 whitespace-nowrap">
												<div className="text-sm text-gray-500">{palet.location}</div>
											</td>
											<td className="px-6 py-4 whitespace-nowrap">
												<div className="text-xs text-gray-400">{palet.id}</div>
											</td>
					...
					{totalPaletPages > 1 && (
						<div className="p-4 border-t border-gray-200 flex justify-between items-center">
							<button
								onClick={() => setCurrentPaletPage(Math.max(1, currentPaletPage - 1))}
								disabled={currentPaletPage === 1}
								className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
							>
								Sebelumnya
							</button>
							<span className="text-sm text-gray-700">
								Halaman {currentPaletPage} dari {totalPaletPages} ({totalPaletItems} palet)
							</span>
							<button
								onClick={() => setCurrentPaletPage(Math.min(totalPaletPages, currentPaletPage + 1))}
								disabled={currentPaletPage === totalPaletPages}
								className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
							>
								Berikutnya
							</button>
				...
				<div className="fixed inset-0 z-[60] overflow-y-auto">
					<div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
						...
						<div className="relative inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
							<form onSubmit={editingPalet ? handleUpdatePalet : handleAddPalet}>
								<div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
									<div className="sm:flex sm:items-start">
										<div className="w-full">
											<div className="flex items-center justify-between mb-6">
												<h3 className="text-2xl font-bold text-gray-900">
													{editingPalet ? 'Edit Palet' : 'Tambah Palet Baru'}
												</h3>
												<button
													type="button"
													onClick={() => {
											...
											<div className="space-y-4">
												<div>
													<label htmlFor="palet-name" className="block text-sm font-semibold text-gray-700 mb-2">
														Nama Palet *
													</label>
													<input
														type="text"
														...
														name="name"
														required
														value={paletFormData.name}
														onChange={handlePaletInputChange}
														...
														placeholder="Masukkan nama palet"
													/>
												</div>

												<div>
													<label htmlFor="palet-location" className="block text-sm font-semibold text-gray-700 mb-2">
														Lokasi *
													</label>
													<input
														type="text"
														...
														name="location"
														required
														value={paletFormData.location}
														onChange={handlePaletInputChange}
														className="w-full px-4 py-3 bg-gray-50 border-0 rounded-2xl text-gray-900 placeholder-gray-400 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all duration-200"
														placeholder="Masukkan lokasi palet"
													/>
												</div>
											</div>
										</div>
								...
								<div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-3xl">
									...
									>
										{isSubmitting ? 'Menyimpan...' : editingPalet ? 'Simpan Perubahan' : 'Simpan Palet'}
									</button>
									<button
										type="button"
										onClick={() => {
							...
							<div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
								<div className="sm:flex sm:items-start">
									...
									<div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
										<h3 className="text-lg leading-6 font-medium text-gray-900">Hapus Palet</h3>
										<div className="mt-2">
											<p className="text-sm text-gray-500">
												Apakah Anda yakin ingin menghapus palet &quot;{paletToDelete?.name}&quot;?
												Semua stok dalam palet ini juga akan dihapus. Tindakan ini tidak dapat dibatalkan.
											</p>
										</div>
									</div>
								</div>
				...
				<div className="fixed inset-0 z-[60] overflow-y-auto">
					<div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
						...
						<div className="relative inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
							<div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
								<div className="flex items-center justify-between mb-6">
									<h3 className="text-2xl font-bold text-gray-900">Scan Barcode Palet</h3>
									<button
										type="button"
										onClick={() => setShowScannerModal(false)}
										className="text-gray-400 hover:text-gray-600 transition-colors"
			...
			{/* Barcode Print Modal */}
			{showBarcodeModal && paletForBarcode && (
				<div className="fixed inset-0 z-[60] overflow-y-auto">
					<div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
						<div
							className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
						...
						<div className="relative inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
							<div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
								...
								<div ref={printRef} className="barcode-container text-center border-2 border-gray-300 p-8 rounded-xl">
									<h1 className="text-3xl font-bold mb-4">{paletForBarcode.name}</h1>
