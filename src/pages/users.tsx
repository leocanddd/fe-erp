...
import { getUsers, User, getRoleName, getRoleColor, registerUser, updateUser, RegisterUserData } from '@/lib/users';
									...
									{Array.isArray(users) && users.map((user) => (
										<tr key={user._id || user.id} className="hover:bg-gray-50">
											...
											<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
												{user.target ? `Rp ${user.target.toLocaleString('id-ID')}` : '-'}
											</td>
											<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
												{user.currentOmset ? `Rp ${user.currentOmset.toLocaleString('id-ID')}` : '-'}
											</td>
				...
				{!loading && users.length > 0 && (
					<div className="mt-6 bg-white/80 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/20">
						<div className="text-sm text-gray-600">
							Total pengguna ditemukan: <span className="font-semibold text-gray-900">{users.length}</span>
						</div>
					</div>
				)}
					...
					<form onSubmit={handleCreateUser} className="space-y-4">
						...
						<div>
							...
							<input
								...
								step="0.01"
								autoComplete="off"
								value={newUserData.target}
								onChange={(e) => setNewUserData(prev => ({ ...prev, target: Number(e.target.value) }))}
								className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900"
							/>
						</div>
