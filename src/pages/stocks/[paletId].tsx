...
export default function PaletDetail() {
	...
	const [error, setError] =
		useState('');
	const [searchStock, setSearchStock] =
		useState('');
	const [
		currentStockPage,
		setCurrentStockPage,
	...
	const handleIncomingStockChange = (
		e: React.ChangeEvent<
			| HTMLInputElement
			| HTMLSelectElement
		>
		...
		setIncomingStockData((prev) => ({
			...prev,
			[name]: value,
		}));
	};

	const handleOutgoingStockChange = (
		e: React.ChangeEvent<
			| HTMLInputElement
			| HTMLSelectElement
		>
	...
	const handleAddIncomingStock = async (
		...
		try {
			const incomingData = {
				productCode:
					incomingStockData.productCode,
				incomingStock: parseInt(
					incomingStockData.incomingStock
				),
			};
	...
	const handleAddOutgoingStock = async (
		e: React.FormEvent
	) => {
		e.preventDefault();
		if (
		...
		try {
			const outgoingData = {
				productCode:
					outgoingStockData.productCode,
				outgoingStock: parseInt(
					outgoingStockData.outgoingStock
				),
			};
					...
					{totalStockPages > 1 && (
						<div className="p-4 border-t border-gray-200 flex justify-between items-center">
							...
							<span className="text-sm text-gray-700">
								Halaman{' '}
								{currentStockPage} dari{' '}
								{totalStockPages} (
								{totalStockItems} item)
							</span>
							<button
								onClick={() =>
									setCurrentStockPage(
										...
										<QRCodeSVG
											value={
												typeof window !== 'undefined'
													? `${window.location.origin}/stocks/${palet.id}`
													: palet.id || 'UNKNOWN'
											}
											size={256}
											level="H"
