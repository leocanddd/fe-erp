...
export const createProduct = async (product: ProductCreateRequest): Promise<ProductResponse> => {
  try {
...
export const updateProduct = async (id: string, product: Partial<ProductCreateRequest>): Promise<ProductResponse> => {
  try {
    const response = await fetch(`${getApiUrl()}/api/products/${id}`, {
      method: 'PUT',
      headers: getAuthHeaders(),
